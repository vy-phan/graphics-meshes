...
void Mesh::computeNormals( MeshNormalStrategy strategy ) {
    ...
    // to face_positions.
    face_normals = face_positions; // same indexes
...
void Mesh::computeNormalsHalfEdge( MeshNormalStrategy strategy ) {
    ...
    for ( int i = 0; i < positions.size(); ++i ) {
        ...
        std::vector< Index > faces = halfedges.vertex_face_neighbors( i );

        for ( const Index& i : faces ) {
            const Triangle t = face_positions.at( i );
            const vec3 a = positions.at( t.A ); // x, y, z of vertex A
            const vec3 b = positions.at( t.B ); // x, y, z of vertex B
            const vec3 c = positions.at( t.C ); // x, y, z of vertex C

            n += normalize( cross( ( a - c ), ( b - c ) ) ); // tally normal
        }

        // Normalize all normals.
        normals.at( i ) = normalize( n );
    }
}

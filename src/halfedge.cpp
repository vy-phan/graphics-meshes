...
void HalfEdgeTriMesh::vertex_face_neighbors( const Index vertex_index, Indices& result ) const
{
    ...
    const HalfEdgeIndex startHedgeIndex = outgoing_halfedge_index_for_vertex( vertex_index );
    HalfEdgeIndex hedgeIndex = startHedgeIndex;

    do {
        // Get the HalfEdge for the half-edge index.
        const HalfEdge& hedge = halfedge( hedgeIndex );

        // Store the face the half-edge points to if it's not -1.
       if (  hedge.face != -1 ) {
           result.push_back( hedge.face );
       }

       // Increment to the next vertex.
       hedgeIndex = halfedge( hedge.opposite_he ).next_he;

    } while( hedgeIndex != startHedgeIndex );
